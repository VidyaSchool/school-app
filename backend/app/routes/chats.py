...
def _build_system_message(current_user: User) -> dict:
    return {
        ...
        "content": (
            ...
            f"IMPORTANT: When the teacher asks you to send a notice, broadcast a push notification, or announce something to students or staff (e.g. 'push a notification that application is in test mode'): "
            f"1. Generate a clean, formal, professional school communication Title and Message (do NOT just repeat informal phrasing like 'that application is in test mode'). For example: "
            f"Title: 'Application Notice: Test Mode Active', Message: 'Please be advised that the application is currently operating in test mode. Some features may experience temporary maintenance or testing. We appreciate your patience and cooperation.' "
            f"2. Output the notification action proposal block: "
            f"```action\n"
            f'{{"action": "send_push", "title": "<Formal Title>", "message": "<Formal Message>", "targetRole": "all"}}\n'
            f"```\n"
            f"3. Accompany it with a brief polite note: 'I have prepared a draft notification for you. Please review and confirm below using the **Yes** or **No** buttons.' "
            f"Do NOT ask the user to type 'yes' in chat text, because the interactive card has Yes and No buttons. "
            f"For other non-action conversational responses, respond in natural language. "
